...
public class UserActionsAdapter extends RecyclerView.Adapter<UserActionsAdapter.ActionViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ActionViewHolder holder, int position) {
        holder.borrowDate.setText(actions[position].BorrowDate.toString());

        if (owner)
            holder.userName.setText(actions[position].BorrowerName);
        else
            holder.userName.setText(actions[position].OwnerName);

        holder.itemName.setText(actions[position].ItemName);
        Picasso.get().load(actions[position].ImagePath).into(holder.itemImage);
    }
